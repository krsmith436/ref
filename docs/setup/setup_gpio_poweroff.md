...
2. The exact millisecond the software finishes halting, the Linux kernel pulls GPIO 24 **LOW (0V)**.
3. This drops the signal to the relay module, immediately **Closing (Turning On) the relay**.
4. At the same moment, the `act-led` controller relinquishes GPIO 23, and the **LED turns completely off**.
